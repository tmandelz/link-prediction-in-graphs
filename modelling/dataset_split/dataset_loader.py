...
class NodePropPredDataset(object):
    def __init__(self, name, root = 'dataset', meta_dict = None):
        ...
        if meta_dict is None:
            ...
            master = pd.read_csv(os.path.join(os.path.dirname(__file__), 'master_node.csv'), index_col=0, keep_default_na=False)
...
class PygLinkPropPredDataset(InMemoryDataset):
    def __init__(self, name, root = 'dataset', transform=None, pre_transform=None, meta_dict = None):
        ...
        if meta_dict is None:
            ...
            master = pd.read_csv(os.path.join(os.path.dirname(__file__), 'master_link.csv'), index_col=0, keep_default_na=False)
            if not self.name in master:
                error_mssg = 'Invalid dataset name {}.\n'.format(self.name)
                error_mssg += 'Available datasets are as follows:\n'
                error_mssg += '\n'.join(master.keys())
